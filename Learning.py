...
def model_learning(model,X,y, fault_len,class_overweight=1, verbose=0):
    ...
    epoch_num = 100 if fault_len else 1
    for epoch_idx in range(epoch_num):
        model.fit_generator(generator(X, y), steps_per_epoch=len(X), epochs=1, verbose=verbose, callbacks=[mc], class_weight=class_weight)
        model.reset_states()
    return model
...
def CRNN_learning(model,gap, win_size):
    ...
    for vnf in vnf_list:
        ...
        for date in date_learning:
            X,y, fault_len= make_data(vnf, fault_, win_size,gap,[date],sliding=5,use_emptylog=True)
            model=model_learning(model, X,y,fault_len,  verbose=1, class_overweight=1)
        test_X, test_y=[], []
        for date in date_test:
            X,y, _= make_data(vnf, fault_, win_size,gap,[date],sliding=5,use_emptylog=True)
            test_X.append(X)
            test_y.append(y)
        Model.evaluation(model,test_X,test_y, rnn_based=True)
